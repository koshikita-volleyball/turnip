...
class Logger {
  _format_log(level: Level, funcName: string, text: string){
    const datetime = dayjs().format('YYYY-MM-DD HH:mm:ss')
    return `【${level}】\t${datetime}\t${funcName}\t${text}`
  }
  _format_slack(level: Level, funcName: string, text: string){
    const dateTime = dayjs().format('YYYY-MM-DD HH:mm:ss')
    const level_str = level === 'info' ? ':large_blue_circle: Info' : ':red_circle: Error'
    return `【Level】${level_str}\n【時刻】${dateTime}\n【関数名】${funcName}\n【メッセージ】${text}`
  }
  log(funcName: string, text: string){
  ...
  error(funcName: string, text: string){
